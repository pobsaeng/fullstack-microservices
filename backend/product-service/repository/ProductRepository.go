...
func UpdateProductStock(code string, amount int) error {
	db := config.Db
	query := `UPDATE tbl_product SET stock = stock + ? WHERE code = ?`

	_, err := db.Exec(query, amount, code)
	if err != nil {
		log.Fatal(err)
		return err
	}
...
func GetProduct(filter string, value interface{}) (*model.Product, error) {
	db := config.Db
	var query string

	// Determine the query based on the filter type (id or code)
	if filter == "id" {
		...
		query = `SELECT id, code, name, description, active, price, stock, weight, brand, color, size, length, width, height, image, category_id, supplier_id, created_by, updated_by, created_at, updated_at FROM tbl_product WHERE code = ?`
	} else {
	...
	// Use QueryRow with the appropriate value
	...
	// Populate product fields based on SQL null checks
	if price.Valid {
		product.Price.SetString(price.String)
	}
	if weight.Valid {
